<div>
    ...
    <div class="">
        ...
        <form action="{{route('admin.berita.update.put', ["id" => $id])}}" class="flex flex-col gap-4" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="max-w-sm">
                <div x-data="{ image: '{{$berita->attachment->path}}', previewImage(event) { 
        ...
        if (file) { 
            ...
            <div class="flex flex-col gap-1">
                ...
                <input type="text" name="title" value="{{$berita->title}}" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    placeholder="Title...">
                @error('title') {{$message}} @enderror
            </div>
            <div wire:ignore>
                <textarea name="content" id="summernote" required>{{$berita->content}}</textarea>
            </div>
